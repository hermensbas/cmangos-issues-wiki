...
While GCC 11.3 is known to work for some users, it is recommended to upgrade your default compiler version to GCC 12 to avoid potential problems:
...
##### Important note: You should not need to use `sudo` in this guide at any point where it isn't explicitly called for in the instructions. If you find that you need to use `sudo` where not instructed to, something probably went wrong. Please check the guide again and make sure you followed it. If you have any questions, feel free to join our discord (links at the top)!
...
* Invoke ++cmake ../mangos++, suggested options are:
  ...
  - Switch compiler (for problems such as the internal compiler error in gcc 11.2):
...
* When finished, move the folders ++maps++, ++dbc++, and ++vmaps++ - optionally ++mmaps++ and ++Cameras++ - that have been created to ++~/cmangos/run/bin++ (the buildings folder is not required and can be deleted).
...
Please skip ahead to <<configuring_cmangos>>!
...
== Configuring CMaNGOS [[configuring_cmangos]]
...
* Getting a compiler error at TileAssembler.cpp:
  - gcc 11.2 (and 11.3 on some distributions) have a bug in their code optimizer. Try to use a different compiler (newer or older version of gcc or clang for example)
