 ...
 * CMAKE is now used to generate visual studio project file as it's are not provided anymore https://cmake.org/[Download it from here].
...
WOWDIR=~/data/org/WoW
rsync -a ~/build/contrib/extractor/ad $WOWDIR/                                                                rsync -a ~/build/contrib/mmap/MoveMapGen $WOWDIR/                                                             rsync -a ~/build/contrib/vmap_assembler/vmap_assembler $WOWDIR/                                               rsync -a ~/build/contrib/vmap_extractor/vmapextract/vmap_extractor $WOWDIR/
rsync -a ~/mangos/contrib/extractor_scripts $WOWDIR/
```
