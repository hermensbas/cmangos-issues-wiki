...
Make sure cmake creates the makefiles for you extractors, I use this line (_-DBUILD_EXTRACTORS_ is the important part):
    cmake ../mangos -DPCH=1 -DDEBUG=0 -DBUILD_PLAYERBOT=ON -DBUILD_EXTRACTORS=ON

If you followed this guide the file can be found in:

* Executables: ++/home/mangos/build/contrib/++

* Scripts: ++/home/mangos/mangos/contrib/extractor_scripts++

To make it easier to copy the scripts you can use this simple script, save it as `copy-extractors.sh`
```sh
#!/bin/sh
# set WOWDIR to wherever your WoW client folder is located. "~" means homedir, so ++/home/mangos++ in the examples. 
WOWDIR=~/data/org/WoW
cp -a ~/build/contrib/extractor/ad $WOWDIR/
cp -a ~/build/contrib/mmap/MoveMapGen $WOWDIR/
cp -a ~/build/contrib/vmap_assembler/vmap_assembler $WOWDIR/
cp -a ~/build/contrib/vmap_extractor/vmapextract/vmap_extractor $WOWDIR/
cp -a ~/mangos/contrib/extractor_scripts $WOWDIR/
```
