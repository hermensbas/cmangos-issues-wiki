...
For other environments (like FreeBSD, ARM and similar) please consider our https://forum.cmangos.net[forum] for assistance.
 ...
 * CMaNGOS: https://cmangos.net[site] | https://forum.cmangos.net[forum] | https://github.com/cmangos[git repositories] | https://discord.gg/Dgzerzb[discord]
...
An alternative page for a detailled installation guide on https://github.com/cmangos/issues/wiki/Detailed-installation-guide-for-Microsoft-Windows[windows installation]
...
### Debian
  ...
  apt install grep build-essential gcc g++ automake git-core autoconf make patch cmake mariadb-dev mariadb-dev-compat mariadb-server libtool libssl-dev binutils zlibc libc6 libbz2-dev subversion libboost-all-dev

### Fedora

  dnf install git cmake gcc patch autoconf mariadb-server mariadb-devel libtool zfstream gcc-c++ subversion boost-devel boost-static

_Note: precompiled headers require a cmake that is version 3.16 or newer. You can compile from source or get binary releases on this page: https://cmake.org/download/
...
We assume you have followed the instructions above, i.e. created the ++mangos++ user and cloned the core repository into ++/home/mangos/mangos++. Adjust the pathes if your setup is different.
...
Note that extracting *mmaps* might take a long time depending on your CPU, you will want to give it as many CPU cores as you can. While *mmaps* are optional, their use is advised, as they improve AI pathfinding. You will be prompeted wether you want to extract them or not by the extraction script.
...
* ++mysql -uroot -p classicmangos < mangos/sql/base/dbc/*.sql++
