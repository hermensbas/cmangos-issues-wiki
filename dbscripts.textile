...
* *dbscript_random_templates*
   ...
   ** WoTLK 20000 +
   ...
   Holds broadcast_text     for type = 0
   Holds dbscripts_on_relay for type = 1

   ** For creature_ai_scripts - ACTION_T_TEXT_NEW.param3 to launch dbscript_random_templates.type = 0, broadcast_text
   ** For creature_ai_scripts - ACTION_T_START_RELAY_SCRIPT to launch type = 2, dbscripts_on_relay
	
   ** For SCRIPT_COMMAND_TALK to launch dbscript_random_templates.type = 0, broadcast_text
...
For *dbscripts_on_creature_movement*, it is the "script id":Creature_movement#script_id of the waypoint.
...
|48|SCRIPT_COMMAND_MODIFY_UNIT_FLAGS|Change Unit flags of a creature.|
...
** dataint - 4: The text ID that the creature will say. See "dbscript_string.":dbscript_string.In case more then only dataint field is filled the texts will be used randomly.
...
** datalong2 = dbscript_random_templates relay Id which should be launched. Takes precedence over relay Id.
...
** Resulting source must be a Unit.
** datalong = SpellType, enum "CurrentSpellTypes":/mangos-wotlk/blob/master/src/game/Entities/Unit.h#L1074
