...
|"BreadcrumbForQuestId":Quest_template#BreadcrumbForQuestId|mediumint(9) unsigned|NO||0||
...
|"RewRepFaction1-5":Quest_template#rewrepfaction|smallint(5)|NO||0||
|"RewRepValue1-5 / RewRepValueId1-5":Quest_template#rewrepvalue|mediumint(9)|NO||0||
...
Class required to get the quest. 0 means available for all classes. Field's value is a decimal value that must be transform to a 8 bits binary in order to be understandable. In binary form, each different bit represent a different class from "CharClasses.dbc":CharClasses.dbc
...
h4. BreadcrumbForQuestId

Breadcrumb Quests that lead to other quests.
...
In wotlk RewRepValueId points towards QuestFactionReward.dbc
...
This is found in the SMSG_QUEST_GIVER_QUEST_DETAILS-packet in sniffs.

h4. DetailsEmoteDelay
...
This is found in the SMSG_QUEST_GIVER_REQUEST_ITEMS-packet in sniffs.

h4. OfferRewardEmote

Emote played by the NPC at the time the character is rewarded for the quest.
This is found in the SMSG_QUEST_GIVER_OFFER_REWARD_MESSAGE-packet in sniffs.

h4. OfferRewardEmoteDelay
